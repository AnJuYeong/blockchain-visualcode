...
// /mnt 밖에까지 나가고
// sudo chmod 777 -R root하고 비밀번호 입력 후
// wsl exit하고 다시 접속하면 됨
// -----------------------------------
...
// snapshot : 데이터를 파일이나 이미지로 저장한다고 보면 됨.

// 이렇게 있고 아무 옵션없이 geth를 실행할 경우
// 디폴트 값이 fast sync가 되고 light sync로 실행 하고 싶으면
// ----------------------------------
// geth --syncmode light
// ----------------------------------

// light sync 동기화할 때는 lightchaindata폴더에 값이 저장된다.
...
// 자바스크립트로 호출해서 사용할 수 있게 끔 만들어 놓은 것.

// personal 모듈중 하나이다.
// personal을 콘솔에 입력
// 속성과 메소드 등등이 쭉 보이게 된다.
// 이것들이 자바스크립트로 호출 할 수 있게 끔 만들어 놓은 것
...
// web3
// web.fromWei(eth.getBalance(account), "ether") : 웨이 -> 이더로 단위 변환
...
// private network를 만드는데 geth의 기능은 사용하지만 최초 블록을 교체해서
// 자체적인 프라이빗 네트워크를 구축할 것.

// json으로도 제네시스 블록의 속성 값을 지정해줄 수 있다.
const genesis = {
    // nonce값을 발견할 난이도 설정
    "difficulty" : "200000",
    // 블록체인 블록당 가스 제한량
...
// 폴더 경로를 go-thereum 폴더로 이동해서
// make all

// puppeth 실행 하고
// 설정값 다 설정하고 파일명이 root98에 .puppeth폴더에 설정한
// 파일 이름으로 json 생성
...
// --newworkId : 체인 아이디와 동일한 값 입력 해주면 된다. 네트워크 아이디
...
// nodejs나 메타마스크에서 프라이빗 네트워크에서 통신하는 것이 가능한 상태
...
// 위에 꺼 안되면 최종본 ㅋ
// 명령어 실행후
// geth --datadir node --http --http.addr "127.0.0.1" --http.port 9000 --http.corsdomain "*" \--http.api "admin,miner,txpool,web3,personal,eth,net" --syncmode full --networkid 7722
