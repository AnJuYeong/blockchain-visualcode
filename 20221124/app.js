// 전에는 solc라이브러리를 사용할 때
// npx solc --bin --abi "sol파일 경로"
// 이렇게 컴파일해서 사용했었는데
// 자바스크립트 구문으로 파일을 컴파일 해보자
...
// 3. 트랜잭션 객체(txObject 변수로 만들고) txObject 객체의 내용에는 data 키 값이
//    있고 data 키 값의 value는 bytecode의 내용

// 4. contract 인스턴스를 만든다.
//    client는 2번 순서에서 만든 web3 인스턴스 속성값을 가지고 만든 client
//    new client.web3.eth.Contract(abi)함수를 사용해서 abi내용을 가지고 있는
//    컨트랙트 인스턴스 생성

// 5. contract.deploy 함수를 사용해서 배포 진행
//    deploy함수가 반환해주는게 promise 객체 트랜잭션 풀이 처리될 때 까지 기다려준다.
//    await contract.deploy(txObject).send({ form : "트랜잭션 발생 시키고 수수료를 낼 계정"})
//    다 트랜잭션이 처리가 되면 배포된 컨트랙트 주소를 가져와서 저장해놓고
//    컨트랙트 주소는 고유의 값이 컨트랙트를 불러올 때 사용한다.
//    contract address는 contract.deploy 함수로 반환 받은 instance객체에서
