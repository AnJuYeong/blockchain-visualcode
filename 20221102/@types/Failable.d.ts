...
// function B() : A<String>{
//     return { value : "asdasdasd"}
// }
// B라는 함수가 있고 T 제네릭을 사용해서 T에 String 타입을 전달했고
// A<T>라는 타입을 반환하는 함수는 B
// 리턴 값은 객체 안에 키값은 value에 value의 타입은 String이다.
// function B() : A<number>{
//     return { value : "123"}
// }
// 생성 시점에 타입이 정해진다. 저렇게 숫자면 숫자를 넣어야함.
...
// D라는 함수가 있고 제네릭을 사용해서 C<T,S> T랑 S가 생성전까지 정해지지 않은 타입
// 그래서 함수의 반환 타입으로 생성할 때 T에 String 그리고 S에 number 타입을 전달하고 생성 해줌으로서
// 함수가 반환하는 타입이 두가지가 될 수 있다. 객체 안에 있는 value의 타입이 문자 타입과 숫자 타입 두가지 
// 타입을 가지고 반환 할 수 있다.