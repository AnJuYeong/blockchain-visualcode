...
export class Block extends BlockHeader implements IBlock{
    ...
    constructor(_previousBlock : Block, _data : string[], _adjustmentBlock : Block){
        ...
        // 테스트쪽에서 생성할 때 블록의 문자열 데이터를 _data이 데이터를 getMerkleRoot함수 안에서
        // _data 블록의 n번째 (문자열)
        // 머클 트리로 뽑고 루트 값을 반환
        ...
        // nonce 값은 마이닝에서 0의 갯수를 난이도에 맞춰서 찾을 때까지 증가 시킬 값
        // 가장 많이 사용하는 해시 알고리즘이 SHA256으로 임의의 데이터를 해시한 값을 구해서
        // 256bit(32바이트)의 값을 얻는 것
        // 해시함수는 단방향 함수로 데이터로부터 해시값을 구할 수는 있는데
        // 해시값으로 데이터를 역산 할 수는 없다. 원본 데이터를 알 수가 없다는 것
        // 해시값이 같은 임의의 데이터를 만들어 내는건 정말 어렵고 (경우의 수를 다 찾는데)
        // 특정한 해시값이 나오는 데이터를 찾으려면 256개(256비트로 나올 수 있는 모든 경우의 수)의
        // 데이터를 다 확인하는데 지금 우리 컴퓨터는 불가능한 작업이다.
        ...
        // 사이에 길이가 512bit 안되면 bit수를 추가해 주는데 zero padding
        ...
        // 이 배열을 반복문으로 돌려서 제곱근을 구하고 정수 값을 제거하고 왼쪽으로 32bit shift 연산 후
        // 16진수로 출력하면
        ...
        // 블록의 생성 시간을 조절해 주는 것 블록이 생성되는 시간을 10마다 비교해서
        // 시간을 조절해 주기 위해서 사용하는 것
    ...
    public static getDifficulty(_newBlock : Block, _adjustmentBlock : Block, _previousBlock : Block) : number{
        // 추가된 블록의 높이가 9이거나 9보다 작으면 난이도를 0으로 고정시키고
        ...
        // 이전 블록의 난이도를 보내주고 (20번 째 블록 전까지 난이도가 이전 블록 난이도인 1이라는 것)
        ...
        // 난이도 조절 식
        // timeToken이 차이값을 구한 시간이 블록의 생성 주기 시간보다 / 2보다 작으면 
        // 난이도를 1올려주고 
        // timeToken이 차이값이 블록 생성 주기 시간 * 2 보다 크면 난이도를 1 내려준다.
        // 이렇게해서 블록의 난이도를 조절해서 블록 생성 주기를 조절해 주는 것
    ...
    public static findBlock(generateBlock : Block){
        ...
        while(true){
            ...
            nonce++;
            // nonce가 올려진 블록 해시를 createBlockHash이 함수로 블록의 모든 내용을 해시화해서
            // 해시값을 얻고
