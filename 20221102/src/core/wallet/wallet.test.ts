...
//          암호화 한 사람과 수신하는 사람 또한 같은 키를 가지고 있어야 한다.

// 비대칭형 : 사용하는 키와 복호화 할 때 사용하는 키가 다른 것 다른 사람한테 절대
//           공개되어서는 안되는 키(비밀키) 비밀키를 토대로 만든 공개키가 한 쌍이다.
//           결론은 키를 2개 사용한다는 것

// 공개키 : 사람들에게 공개할 키 정보를 암호화 할 수 있다.
// 비밀키 : 사용자만 알아야 하는 키 암호를 풀 수 있는 키
...
// 비대칭형 키의 경우는 암호화 할 때 공개키로 암호화하고

// 나 -> 원본 -> 너의 공개키로 암호화 -> 암호문
// 너 -> 암호문 -> 너의 비밀키로 복호화 -> 원본

// 비밀키의 소유자가 비밀키로 원본을 암호화 하고 공개키와 함께 전달한다.
// 공개키와 데이터를 획득한 사람은 공개키를 이용해서 복호화가 가능하고
// 이렇게 위험성이 보이는데 이 방법을 사용하는 이유가 데이터의 보호 목적보다는
// 공개키로 데이터 제공한 사람의 신원을 보장해주기 때문
...
// 실제 블록체인 네트워크 상에 개인키로 생성하는 방식은
// 256 자리의 2진수로 랜덤 값을 64자리의 16진수 값으로 만든 것이 개인키이고
...
describe('지갑 만들기', () => {
    ...
    it("지갑 주소", () => {
        ...
        // 변경할 수 없다.
        // 데이터를 담아놓고 사용할 시기를 제어할 수 있는
        // node.js에서 데이터를 전송하는 속도같은 부분을 제어하기 위해서  사용한다.
        //
        ...
        // 24자리를 잘라내고 40자리만큼 남겨주면 된다.
        // slice(26)을 넣은 이유가 elliptic를 이용해서 만든 공개키의 앞자리
        // 02 혹은 03이 붙어서 이값도 제거하기 위함.
        const addres = buffer.slice(26).toString();
        console.log("계정 : " + addres);
    })

    // 동일한 내용을 "transaction data"이 값으로 해시값을 만들어서 검증에 사용한 것이기 때문에
    // 검증에 성공한 것 true반환
    ...
    // 쉽게 만들 수 있게 함수를 제공해주는 라이브러리를 사용하는 것
    // 설치 명령어
    // ---------------------------------------------------
