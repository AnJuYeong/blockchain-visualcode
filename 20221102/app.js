...
// 머클 트리란 머클 트리는 수많은 트랜젝션을 각각 해싱해서
// 2개의 짝 지어서 해싱을 계속 반복해서 최종적으로 하나가 남을 때 까지 행싱한 트리
// 1. 첫번째 데이터를 SHA256형태의 해시값으로 변환
// 2. 2개의 노드를 한 쌍으로 묶어서 해시값 변환
// 3. 해시값으로 변환을 반복해서 마지막 하나가 남을 때 까지 반복
// 4. 블록의 모든 거래를 합친 해시값이 머클 루트 
// 머클 트리가 필요한 이유는 머클 트리의 목적이
// 데이터의 간편성과 확실한 인증이기 때문에

// 블록체인 네트워크 정보를 모두 다운로드하면 무척 용량이 크기 때문에
...
// 아니면 블록의 값이 변조되면 머클 루트 해시 값이 변조되고
// 잘못된 해시 값이 나오게 되면 해당 블록이 잘못된 것으로 블록을 거부하고
...
// 비트코인 네트워크 트랜잭션
// 블록이 생성될 때 블록안에 data 안에 트랜잭션을 담아준다.

// 블록이 생성되면 data에 트랜잭션을 담아주는데 이게 첫 번째 트랜잭션은 코인베이스 트랜잭션
// 이라고 부르고 내용은 마이닝을 했을 때 얻는 보상의 내용이 들어간다.
// 블록을 채굴하고  채굴한 사람이 얼마나 코인을 보상 받았는지.

// 트랜잭션 내용 안에 객체가 표현되어 있고
// UnspentTransactionOutput(UTXO)이 공간에 잔액을 객체로 저장
// 암호화폐에서 아직 사용되지 않은 거래 출력 사용할 수 있게 승인된 일정량의 디지털 화폐
// 사용되지 않은 미사용 객체
...
// UTXO에서 {주소 : 누구, 코인 : 50, 참조 트랜잭션 : Tx 0000}를 제거하고
// UTXO = [{ 주소 : 너, 코인 : 1, 참조 트랜잭션 : Tx 0001}, 
//          { 주소 : 누구, 코인 : 49, 참조 트랜잭션  : Tx 0001}]

// 누구라는 사용자가 "너"에게 50코인에서 1코인을 보낸다고 가정해서
// input에 Tx 0000 트랜젹션을 속성 값으로 전달하고 output에는 두 객체
//  { 주소 :  너, 코인 : 1, 참조 트랜잭션 : Tx 0001},
//  { 주소 : 누구, 코인 : 49, 참조 트랜잭션  : Tx 0001} 객체가 만들어지고
...
// Tx 0003 = {
//  input : [{ 주소 : 너, 코인 : 49, 참조 트랜잭션 : Tx 0001 }, { 주소 : 너, 코인 : 20, 참조 트랜잭션 : Tx 0002 }]
//  output : [{ 주소 : 누구, 코인 : 21 }]
// }
...
// A가 B한테 10 BTC의 전송을 보내면 지갑 서버에서 요청을 받아서
// 서명과 트랜잭션 데이터를 만들고 블록체인 서버쪽에 해당 데이터를 전송
// 블록체인 서버에서 트랜잭션 객체를 만들어 준다.

// 트랜잭션 객체가 생겼을 때 UTXO 배열 안에서 unspentTxOut 객체를 제거하고 트랜잭션을 통해
// 새로 생긴 unspentTxOut 객체들을 UTXO에 추가 시켜주는 작업을 하고
// UTXO 내용을 업데이트 해준다.

// 트랜잭션 풀 이라는 공간을 만들고 트랜잭션 객체가 생성됐을 때 만들어진 트랜잭션을
// 트랜잭션 풀에 담아준다. 그리고 블록체인 네트워크 상에 연결된 노드들은 트랜잭션 풀의 내용을
// 공유한다. 블록 마이닝할 때 트랜잭션 풀 안에 있는 트랜잭션 객체를 사용해 새로 생선되는 블록의
// data 속성 값으로 넣어준다.
// 그래서 필요한게 블록이 마이닝될 때 트랜잭션 풀을 업데이트 해주는 부분이 필요
...
// 컴퓨터에 메모리나 CPU 등등 지원을 쪼개서 OS를 따로 설치해서 사용할 수 있는데
// 이러면 컴퓨터 성능이 줄어들어 비효율적인 단점이 있다.
// WSL은 이 단점을 보완하고 개발 환경을 쉽게 구축 가능

// WSL2를 설치해서 사용할거다
// 윈도우에서 Linux 명령어를 사용할 수 있게 도와주는 툴(WSL2)

// 제어판 -> 프로그램 -> 프로그램 및 기능 -> windows기능 켜키/끄기
