...
// 네이버같은 페이지만 봐도 비밀번호 찾기를 시도할 떄
// 비밀번호를 알려주지 않고 비밀번호 변경을 시켜준다.

// 복호화는 암호문을 편문으로 변환하는 과정
// 부호화(인코딩)된 데이터를 부호화 되기 전 형태로 바꿔서 
// 사람이 읽을 수 있는 형태로 되돌려 놓는 것.
...
// 456456465랑 12356453451341둘의 길이가 다른데
...
// 사용할 해시 알고리즘은 sha512 암호 알고리즘을 사용할 건데
// (md5, sha1, sha256, sh512등이 있는데)
// sha-512알고리즘은 국가안보국(NSA)이 설계한 암호 해쉬함수이다.
// sha512는 512비트(64바이트) 해시 값을 만들어주는데
// 일반적으로 길이가 128자리인 16진수로 렌더링 된다.

// 선택된 알고리즘으로 해싱한다.
let hashing = hashAlgor.update(pw);// 매개변수로 암호화 시킬 문자를 넣는다.
...
// salt 값을 잘 보관해 놓고 숨겨놓자 .env

// 솔트 값을 만들어보자. 소금 생성
// 크립트의 랜덤 바이트 생성 함수 랜덤한 바이트를 생성 시킬 수 있다.
...
// pbkdf, scrypto, bcrypto 이 세가지 방법이 있는데,
    ...
    return new Promise((resolve, reject) => {
        ...
        client.query(sql, [userId],async(err, result) => {
            ...
            // 여기서 결과값은 해당 유저의 객체고 그 안에 salt 값을 가져온다.
                ...
                // 길이 : 64, 반복 횟수 : 14
                crypto.pbkdf2(userPw, salt, 14, 64, "sha512", (err, key) => {
...
const createPwHashed = (userPw) => {
    // 비동기 처리
    return new Promise(async(resolve, reject) => {
        const salt = await createSalt();// 여기소 salt 값을 만들고
        // 여기서 14 반복시키는데 키 스트레칭
        // 비밀번호에 문자를 더해서 암호화 시키는 기법 salt 사용
        ...
        crypto.pbkdf2(userPw, salt, 14, 64, "sha512",(err, key) => {
            if(err){
                ...
                // 암호화한 비밀번호와 salt 값을 둘다 데이터 베이스에 저장
                resolve({userPw: key.toString("base64"), salt});
            }
        });
    });
}
...
// express 열고, 데이터베이스연결까지
// 데이터베이스 이름은 test8으로
// express에서 body객체 사용할거다.
...
app.post("/join", async(req, res) => {
    const { userPw, salt } = await createPwHashed(req.body.user_pw);
    const sql = "INSERT INTO users (user_id, password, salt)VALUES(?,?,?)";
    client.query(sql,[req.body.user_id,userPw, salt], () => {
        res.redirect("/login");
    });
});
