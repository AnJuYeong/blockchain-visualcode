...
// 요청을 메타마스크가 받아서 사용자의 개인키를 이용해 서명을 만들어 트랜잭션을 발생 시킨 후 블록체인 네트워크로 요청을 보낸다.
// 메타마스크는 프론트에서 보낸 요청을 트랜잭션을 발생시켜 블록체인 네트워크로 보낸다는 것

// 요청
// 프론트 -> input -> 메타마스크(지갑) -> 블록체인 네트워크
// 메타마스크 : 사용자의 개인키를 이용해서 서명 만들고 전달받은 데이터와 서명을 합쳐서 블록체인 네트워크로 전달 
...
// 쇼핑몰을 예를 들어 상품들이 등록되어 있고 사용자가 코인이나 토큰으로 구매를 할 떄 
// 상품데이터 까지 블록체인 네트워크에 저장하면 너무 비효율적이기 때문에 
// 프론트, 백엔드 같이 구현해서 상품 데이터는 데이터 베이스에 저장하고 상품 목록을 불러오고 특정 상품의 정보들을 가져올땐 백엔드 서버에 요청해서 가져오는 방식

// 프론트에서 메타마스크로 바로 요청을 보내서 트랜잭션을 발생시키는 방법과 
// 백엔드에서 서명을 제외한 트랜잭션 객체를 만들어서 프론트에 전달하고 프론트에서 
// 백엔드로 부터 받은 트랜잭션 객체를 메타마스크로 전달해 서명과 함께 트랜잭션을 보내는방법
// 백엔드에서 사용자의 개인키를 가지고 서명을 만들어서 서명까지 포함된 트랜잭션을 블록체인 네트워크로 바로 보내는 방법

// 중요한건 사용자의 개인키로 서명을 만드는게 우리가 아니고 서명을 만들어주는건 메타마스크가 된다는 것