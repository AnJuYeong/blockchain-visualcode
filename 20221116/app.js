...
// /home/root98/Ethereum/go-ethereum/build/bin

// export PATH=$PATH:/home/root98/Ethereum/go-ethereum/build/bin
...
// 코인을 받고 물건을 전달하는 거래 뿐만이 아니라 조건에 따라 특정한 코드가 실행되어서
...
// EVM은 가상머신이고 정해진 스마트 컨트랙트 규칙에 따라서 코드를(계약) 실행하고 
...
// EOA로 트랜잭션을 시작할 수 있다.
// EOA는 다른 EOA 또는 CA에 메시지를 보낼 수 있으며 이 동작은 개인키를 사용해서 트랜잭션을 만들고
// 서명하고 두 개의 EOA사이에서 발생하는 메시지는 단순히 ETH만을 전송
...
// CA는 EOA와 다른게 개인키를 가지고 있지 않고 스스로 트랜잭션을 생성 할 수도 없다.
// CA는 다른 CA 또는 EOA로 부터 받은 트랜잭션에 대한 응답으로만 트랜잭션을 실행 할 수 있다.
...
// 가스는 이더리움 네트워크에서 트랜잭션을 생성할 때 보내는 사람이 부담하는 수수료 같은 개념

// Web3라이브러리를 사용하면 노드간의 통신을 할 때
// RPC라는 개념
// RPC라는 개념이 나온 이유
...
// 서버는 그 데이터를 받고 요청 받은 응답을 반환해주고
// 클라이언트는 서버로부터 응답 값을 받아서 본인의 요청을 결과로 받을 수 있다.
// http socket

// RPC는 원격 프로시저 호출 별도의 원격제어를 위한 코딩없이 다른 주소 공간에서
...
// curl -X POST -H "Content-type : application/json" --data '{"jsonrpc" : "2.0", "method" : "eth_getBalance", "params" : ["0xb0da730cc19403922ed616788a54b23297869cc4","latest"]}' http://localhost:8545
// curl -X POST -H "Content-type:application/json" --data '{"jsonrpc" : "2.0","method" : "eth_getBalance", "params" : ["0xf80F0c949AFB9028315AF2fC0f1F0beD5Fb5D581","latest"]}' http://localhost:8545

// 이더리움 클라이언트에게 RPC를 요청 보내는 방법이다.

// Web3 라이브러리
// Web3.js라는 라이브러리를 제공 받아서 이더리움 네트워크와 상호작용을 할 수 있는 다양한 함수들
// 제공 받아 사용할 수 있다.
