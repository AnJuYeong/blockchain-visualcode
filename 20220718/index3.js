...
// 노드 모듈 폴더는 git에 따로 올리지않고
// package.json만 올리고
// npm i나 npm install로 설치후 작업한다.

// package-lock.json에 기록되어 있는 내용은 실제로 설치된 패키지들이 어떤 것인지 알려준다.
// 팀에서 이 프로젝트를 같이 작업을 한다. lock.json도 같이 업로드 해주는 것이 좋다.
// package.json에는 패키지의 버전 앞에  "prettier": "^2.7.1" // 웃음 표시가 있는데
// 이 기호가 있으면 버전이 정확하지 않아도 설치되게 만들어준다.

// 서로 팀원끼리 차이가 날 수 있어요.
// 그래서 여러명과 같이 작업할 때 lock.json파일을 서로 공유해야 하는 이유
// 버전이 맞지 않는 문제를 해결할 수 있다.

// node.modules파일도 생겼는데 npm 설치를 하니깐 이 폴더 안에 설치되어 있는
