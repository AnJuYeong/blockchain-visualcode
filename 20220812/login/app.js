...
const  app = express();
// req.body
// express 12버전쯤인가 버전업 되면서 express 설정으로 body 객체를 사용하게 설정할 수 있다.
app.use(express.urlencoded({extended : false}));
// 세션도 사용할거니깐
app.use(
    session({
        // 세션 발급할 때 사용되는 키 노출되면 안되니깐 .env 파일에 값을 저장해놓고 사용 process.env.SESSION_KEY
        // 세션 발급할 때 사용할 키
...
app.get("/", (req, res) => {
    fs.readFile("view/login.html", "utf-8", (err, data) => {
        // login.html 파일을 utf-8 인코딩을해서 send함수를 이용하여 data를 보내줌(요청을 응답해줌)
        res.send(data);
    });
});
...
app.get("/join",(req, res) =>{
    fs.readFile("view/join.html", "utf-8", (err, data) => {
        // join.html 파일을 utf-8 인코딩을해서 send함수를 이용해서 data를 보내줌(요청을 응답해줌)
        res.send(data);
    });
});
...
// // user_id 이름으로 컬럼을 만들고 VARCHAR(255) 255자까지 허용
...
app.post("/join", (req, res) => {
    // req.body 객체에 있는 키 값으로 변수에 할당
    // req.body.userId가 userId에 담긴다.
    // req.body.userPw가 userPw에 담긴다.
    // {요 안에 키값} 객체 구문으로 묶어서 변수를 받으면 해당 객체의 키 값의 벨류를 받을 수 있다.
...
// 로그인이 유지되고 있는 동안에만 동작해야 하는 페이지들이 있는데. 로그인 유지들 확인하고 요청을 보내야한다..
// 어떻게 해야하나..
...
// 중간에 문지기 얘의 허락을 맡아야 지나갈 수 있다. 엑세스 권한
// req(요청)객체, res(응답)객체, next() 함수를 이용해서 통과 요청을 넘길 수 있다. 너 지나가 = next();
// 문지기 통과 next 지나가세요
// 요청을 처리하기전에 중간에 기능을 동작 시켜주는 애

// 매개변수는 (요청 객체, 응답 객체, next 함수)
const middleware = (req,res,next) => {
    // 세션값을 가져온다.
    // const accessToken = req.session.access_token;
    // const refreshToken = req.session.refresh_token;
    // 두줄을 한줄로 줄이면 밑에처럼 사용할 수 있다.
    const {access_token,refresh_token} = req.session;
    // access_token 값을 먼저 검증한다. 유효기간이 끝나지 않았는지 안썩었는지
...
// middleware이 미들웨어 함수에서 next()함수를 사용하지 않으면
// 다음 콜백함수는 실행되지 않는다.
// 문지가한테 막힌거임
